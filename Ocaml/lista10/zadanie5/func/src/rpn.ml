...
let stack_size (rpnn : prog) : int =
  let rec size_rpn (rpn : prog) (stack : prog) (cur : int) (mx : int) : int =
    match rpn with
    | PushInt n :: rpntail ->
      let new_stack, new_cur = reduce_and_size (PushInt n :: stack) (cur+1) in
      size_rpn rpntail new_stack new_cur (max (cur+1) mx)
    | PushVar _ :: rpntail ->
      let new_stack, new_cur = reduce_and_size (PushInt 1 :: stack) (cur+1) in
      size_rpn rpntail new_stack new_cur (max (cur+1) mx)
    | Binop op :: rpntail ->
      size_rpn rpntail (Binop op :: stack) (cur+1) (max (cur+1) mx)
    | [] ->
