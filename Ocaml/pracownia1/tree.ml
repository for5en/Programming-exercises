...
let isok (t : 'a tree) : bool = 
  match t with
  | Leaf -> true
  | Node(l, v, r) ->
    let sst = size t in
    let ssl = size l in
    if (float_of_int ssl) <= alpha *. (float_of_int sst) && (float_of_int (sst - ssl - 1)) <= alpha *. (float_of_int sst) then true
    else false

let insert (x : 'a) (sgt : 'a sgtree) : 'a sgtree =
  if find x sgt then sgt
  else
    let rec recins (t : 'a tree) : 'a tree =
      match t with
      | Leaf -> Node(Leaf, x, Leaf)
      | Node(l,v,r) ->
        if x < v then 
          let newl = recins l in
          if isok (Node(newl, v, r)) then Node(newl, v, r)
          else rebuild_balanced (Node(newl, v, r))
        else
          let newr = recins r in
          if isok (Node(l, v, newr)) then (Node(l, v, newr))
          else rebuild_balanced (Node(l, v, newr))
    in 
    let ntree = recins sgt.tree in
...
let remove (x : 'a) (sgt : 'a sgtree) : 'a sgtree =
  if not (find x sgt) then sgt
    ...
    let rec recrem (t : 'a tree) (y : 'a) : 'a tree =
      ...
      | Leaf -> Leaf
      | Node(l, v, r) ->
        if x < v then Node(recrem l y, v, r)
        else if v < x then Node(l, v, recrem r y)
        else 
          (
          match (l, r) with
          | (Leaf, Leaf) -> Leaf
