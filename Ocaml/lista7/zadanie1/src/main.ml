...
let rec eval (e : expr) : value =
  ...
  | Bool b -> VBool b
  | Binop (op, e1, e2) ->
    (match op, eval e1, e2 with
    | And, VBool v1, _ -> if v1 then (eval e2) else (VBool false)
    | Or, VBool v1, _ -> if v1 then (VBool true) else (eval e2)
    | _, v1, _ -> eval_op op v1 (eval e2))
