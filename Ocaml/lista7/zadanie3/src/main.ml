...
let rec foldlist (e : expr) (x : ident) (y : ident) (e1 : expr) (e2 : expr) : expr = 
  match e with
  | Unit -> e2
  | Pair(p1, p2) -> subst y (foldlist p2 x y e1 e2) (subst x p1 e1)
  ...
  | Match (e1, x, y, e2) ->
      (match eval e1 with
        | VPair (v1, v2) ->
            ...
            |> eval
        | _ -> failwith "Type error")
  | Fold (e, x, y, e1, e2) -> eval (foldlist e x y e1 e2)
